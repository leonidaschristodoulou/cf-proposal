...
class MCCE:
    ...
    def fit(self, 
        ...
        visited_columns = []
        for col, _ in self.visit_sequence.sort_values().items():
    ...
    def _fit(self, 
        ...
        for col, _ in self.visit_sequence.sort_values().items():
            ...
            # No predictors available: fall back to marginal sampling instead of CART
            if len(col_predictors) == 0 and self.method[col] == 'cart':
                col_method = METHODS_MAP['sample'](dtype=self.df_dtypes[col], random_state=self.seed)
                self.method[col] = 'sample'
            ...
            if self.method[col] == 'cart':
                ...
                self.predicted_leaves[col] = col_method.fitted_model.apply(df[col_predictors].to_numpy())
    ...
    def postprocess(self, 
        ...
        for idx in list(set(self.results.index)):
            ...
            _sparse_rows.append(close_df)

        results_sparse = pd.concat(_sparse_rows, axis=0) if _sparse_rows else pd.DataFrame(columns=self.results.columns)
