...
def distance(cfs, 
    ...
    continuous = list(dataset.continuous)
    categorical = list(dataset.categorical)
    categorical_encoded = list(getattr(dataset, 'categorical_encoded', []))
    if not categorical_encoded and hasattr(dataset, 'encoder'):
        categorical_encoded = dataset.encoder.get_feature_names(categorical).tolist()

    target = getattr(dataset, 'target', None)
    if target and target in continuous:
        continuous.remove(target)
    if target and target in categorical:
        categorical.remove(target)
    if target and target in categorical_encoded:
        categorical_encoded.remove(target)
    ...
    L0 = np.sum(np.invert(np.isclose(delta, np.zeros_like(delta), atol=1e-05)), axis=1).astype(float).tolist()
    L1 = np.sum(np.abs(delta), axis=1).astype(float).tolist()
    L2 = np.sum(np.square(np.abs(delta)), axis=1).astype(float).tolist()
