# ==========================================================================
# E2: full-protocol NICE-sparse vs TabPFN for the 3 remaining hung combos --
# australian, blood_transfusion, chronic_kidney_disease -- under the same
# agreed 120s-per-instance / 120s-build cap already used for
# diabetes_binarized/heloc (e2_regen_nice_spars_tabpfn_unskipped_v2.py).
#
# Prior diagnostics (REVISION_EVIDENCE_LOG.md -> "E2") used different, looser
# budgets (a single uncapped ~2h40min kill for australian; a 300s
# per-dataset kill for blood_transfusion/chronic_kidney_disease) and marked
# all three `not_run` on that basis. Re-running under the actual decided
# 120s cap, mechanically, is what E2/E3 asks for -- no per-dataset judgement
# calls. Two of the three (blood_transfusion, chronic_kidney_disease) hang
# during explainer CONSTRUCTION, which this harness bounds cheaply: a
# construction hang marks the WHOLE (dataset, seed) pair as one
# "build_timeout" row in exactly 120s, not per-instance -- so those two cost
# ~2 x 5 x 120s = 1200s total. australian hung on the first per-instance
# call in the earlier diagnostic (construction succeeded); if that turns out
# to affect every factual, per-instance retries make it the expensive case
# (worst-case ~5 x 100 x 120s = 60000s = 16.7h) -- still bounded, unlike the
# original uncapped hang, and checkpointed per pair so a partial run is
# resumable.
#
# Identical mechanism to the v2 script for diabetes_binarized/heloc: a
# persistent worker subprocess builds the explainer once, then processes
# factuals one at a time over a Queue; a construction hang or a per-instance
# stall past the timeout kills only that worker and is recorded, without
# losing already-completed work.
#
# Writes to a NEW file (nice_spars_tabpfn_remaining3.joblib) -- does not
# touch any existing joblib cache, including nice_spars_tabpfn_unskipped_v2.joblib.
# Checkpointed per (dataset, seed) pair.
# ==========================================================================
...
DATASETS = ["australian", "blood_transfusion", "chronic_kidney_disease"]
LOAD_NAME_MAP = {}
SEEDS = [1, 3, 5, 7, 9]
N_FACTUAL = 100
PER_INSTANCE_TIMEOUT_S = 120       # the agreed common rule
BUILD_TIMEOUT_S = 120              # explainer construction gets the same budget
CHECKPOINT_PATH = "/nvme/h/lchristodoulou/pace/cf-proposal/nice_spars_tabpfn_remaining3.joblib"
